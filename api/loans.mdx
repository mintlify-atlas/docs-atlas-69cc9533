...
sidebarTitle: "Loans"
description: "Reference for loans.php endpoints: POST to create a loan, GET to retrieve by CI or admin views, PUT to register a return or update loan details. Includes request/response schemas."
---
...
| 404    | `{ "error": "Préstamo no encontrado o ya devuelto" }`           | Loan not found or already in target state     |
| 500    | `{ "error": "Error al guardar: ..." }`                          | Database write failed                         |
...
<ResponseField name="today" type="integer">
  Number of loans created today (based on `DATE(checkout_time) = CURDATE()`).
</ResponseField>
...
<ResponseField name="returned_today" type="integer">
  Number of loans returned today (based on `DATE(return_time) = CURDATE()`).
</ResponseField>
